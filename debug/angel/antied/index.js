...
function fluxDispatchPatch(getDeletedMessageArray) {
    ...
    if (exports.isEnabled) {
      try {
        ...
        if (ev.type === "MESSAGE_DELETE") {
          ...
          plugin.storage.deletedMessages = deletedMessages;
          return args;
        }
...
const getDeletedMessageArray = function() {
  if (!plugin.storage.deletedMessages) {
    plugin.storage.deletedMessages = {};
  }
  return plugin.storage.deletedMessages;
};
let unpatch = null;
let intervalPurge;
...
var index = {
  onLoad: async function() {
    ...
    intervalPurge = setInterval(function() {
      ...
      if (keys.length <= KEEP_NEWEST)
        return;
      const toDelete = Math.min(DELETE_EACH_CYCLE, keys.length - KEEP_NEWEST);
      keys.sort(function(a, b) {
        return (deletedMessages[a].timestamp || 0) - (deletedMessages[b].timestamp || 0);
      }).slice(0, toDelete).forEach(function(key) {
  ...
  */
  onUnload: function() {
    ...
    unpatch?.();
    _vendetta.logger.info("[ANTIED] Plugin unloaded. Messages saved in persistent storage.");
  },
